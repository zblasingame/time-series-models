...
def build_residual_block(X, num_inputs):
    ...
    f = tf.nn.relu_layer(X, w1, b1)

    return tf.nn.relu(tf.add(tf.add(tf.matmul(f, w2), b2), X))


def build_fc_layer(X, act, input_dim, output_dim):
